export type CreatureKey = "lanternfish"|"goliath_grouper"|"dungeness_crab"|"blue_crab"|"great_white_shark"|"giant_otter"|"horrid_stonefish"|"peacock_mantis_shrimp"|"blobfish"|"european_seabass"|"atlantic_salmon"|"skipjack_tuna"|"pink_shrimp"|"atlantic_cod"|"maine_lobster"|"sea_otter"|"stubby_squid"|"crown_of_thorns";
...
function powerToTier(basePower: number): "S" | "A" | "B" | "C" | "D" | "F" {
  if (basePower >= 900) return "S";
  if (basePower >= 600) return "A";
  if (basePower >= 300) return "B";
  if (basePower >= 150) return "C";
  if (basePower >= 50) return "D";
  return "F";
}

function prevalenceToTier(
  prevalence: number
): "Common" | "Rare" | "Epic" | "Legendary" {
  if (prevalence >= 0.7) return "Common";
  if (prevalence >= 0.4) return "Rare";
  if (prevalence >= 0.15) return "Epic";
  return "Legendary";
}

// Choose a random creature based on prevalence
export function randomCreature() {
  const keys = Object.keys(CREATURES) as CreatureKey[];

  // Step 1: build a list of weights (prevalence values)
  const weights = keys.map((k) => CREATURES[k].prevalence);
  ...
  for (let i = 0; i < keys.length; i++) {
    ...
    if (r <= 0) {
      ...
      return { 
        key: k, 
        powerTier: powerToTier(c.basePower), 
        prevalenceTier: prevalenceToTier(c.prevalence), 
        ...CREATURES[k] 
      };
    }
  }
